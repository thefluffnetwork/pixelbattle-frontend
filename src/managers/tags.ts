...
export const TagsManager = {
  ...
  async fetch() {
    ...
    const isUserSelectedTagFake = !TagsManager.tags.value.some(
      tag => tag.name === TagsManager.selectedTag.value,
    )
  ...
  purgeFakeTags() {
    TagsManager.tags.value = TagsManager.tags.value.filter(
      tag => tag.pixels !== -1,
    )
  },
  select(name: string) {
    ...
    const isPreviousTagFake =
      TagsManager.tags.value.find(tag => tag.name === previousTag)?.pixels ===
      -1
    if (isPreviousTagFake) {
      TagsManager.purgeFakeTags()
    }

    const isCurrentTagReal = TagsManager.tags.value.find(
      tag => tag.name === name,
    )
    if (!isCurrentTagReal) {
