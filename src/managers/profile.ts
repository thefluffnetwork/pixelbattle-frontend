...
import { type ProfileInfo, UserRole } from "../interfaces/Profile"
...
export const ProfileManager = {
  ...
  load() {
    ...
    if (token && id) ProfileManager.profile.value = { token, id }
  },
  async fetch() {
...
ProfileManager.isStaff = computed(
  () =>
    (ProfileManager.user.value?.role ?? UserRole.User) >= UserRole.Moderator,
)
