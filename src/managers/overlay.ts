...
export const OverlayManager = {
  ...
  async load() {
    ...
    if (!localStorageOverlay) return

    OverlayManager.image.value = await new AppImage(
      await stringToBlob(localStorageOverlay.data),
    ).process()
    OverlayManager.position.value = new Point(
      localStorageOverlay.position.x,
      localStorageOverlay.position.y,
    )
    OverlayManager.imageName.value = localStorageOverlay.name
    OverlayManager.opacity.value =
      localStorageOverlay.opacity ?? config.overlay.defaultOpacity
  },
}
